...
export default function TrainerProgramsPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", durationWeeks: "" });
  ...
  const createProgram = useMutation({
    mutationFn: (data: { name: string; description?: string; durationWeeks?: number }) =>
      api.post("/api/programs", data),
    onSuccess: () => {
      toast.success("Program created");
      setCreateOpen(false);
      setForm({ name: "", description: "", durationWeeks: "" });
  ...
  const assignProgram = useMutation({
    mutationFn: ({ programId, memberId }: { programId: string; memberId: string }) =>
      api.post("/api/programs/assign", { programId, memberId }),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">Create programs and assign to members</p>
        </div>
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Define a new workout program for your clients</DialogDescription>
            </DialogHeader>
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="3-day split focusing on compound movements..."
            ...
            <Card key={prog._id} className="transition-all hover:border-[var(--primary)]/30">
              <CardContent className="p-6">
                ...
                <p className="mt-2 text-xs text-[var(--muted-foreground)]">
                  {prog.durationWeeks ? `${prog.durationWeeks} weeks` : "No duration set"} · {formatDate(prog.createdAt)}
                </p>
                <Button
                  variant="outline"
                  size="sm"
